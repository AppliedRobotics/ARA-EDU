...
# Конфигурация Wi-Fi точки доступа
AP_SSID = "ESP32-Camera-AP"
...
stream_client = None
FRAME_INTERVAL = 1000 // 30  # 10 FPS

MSP_MAX_SUPPORTED_CHANNELS = 16
rc_motors = [1500, 1500, 1000, 1500] + [1000] * (MSP_MAX_SUPPORTED_CHANNELS - 4)  # Roll, Pitch, Throttle, Yaw + остальные

# ========== ВАШ СУЩЕСТВУЮЩИЙ КОД MSP/IMU ==========
MSP_SET_RAW_RC = 200
MSP_RAW_IMU = 102
...
def create_msp_request(command, payload=b''):
    size = len(payload)
    packet_parts = []
    packet_parts.append(b'$M')
    packet_parts.append(b'<')
    packet_parts.append(bytes([size]))
    packet_parts.append(bytes([command]))
    ...
    for byte in payload:
        crc ^= byte
    packet_parts.append(bytes([crc]))
    
    return b''.join(packet_parts)


def read_serial_non_blocking():
    """Оптимизированное чтение сериала с буферизацией"""
    data = b''
    max_reads = 100  # Защита от зацикливания
    ...
    if data:
        ...
        while start_idx < len(data):
            ...
            pkt_size = data[idx + 3]
            total_size = 6 + pkt_size  # $M< + size + cmd + payload + crc
            
            if idx + total_size <= len(data):
                packets.append(data[idx:idx + total_size])
                start_idx = idx + total_size
            else:
                break
...
def parse_msp_response(data):
    ...
    for i in range(len(data) - 4):
        if data[i:i+2] == b'$M' and data[i+2:i+3] == b'>':
            start_idx = i
            break
...
def bytes_to_int16_le(data, offset):
    try:
...
def bytes_to_int32_le(data, offset):
    try:
...
def parse_imu_data(payload):
    ...
    try:
        accel_x = bytes_to_int16_le(payload, 0)
        accel_y = bytes_to_int16_le(payload, 2)
        accel_z = bytes_to_int16_le(payload, 4)
...
def rc_control_task():
    """Задача управления RC командами"""
    global blink_enabled, motor_enabled, action_counter, rc_motors
    
    # Управление миганием
    if blink_enabled:
        rc_motors[4] = 2000  # ARM on
        if action_counter >= ACTION_DURATION:
            blink_enabled = False
            action_counter = 0
            rc_motors[2] = 1000  # Throttle off
            rc_motors[4] = 1000  # ARM off
        action_counter += 1
    
    # Управление моторами
    elif motor_enabled:
        rc_motors[4] = 2000  # ARM on
        rc_motors[2] = 1050  # Throttle on
        if action_counter >= ACTION_DURATION:
            motor_enabled = False
            action_counter = 0
            rc_motors[2] = 1000  # Throttle off
    ...
    rc_motors[0] = 1500  # Roll neutral
    rc_motors[1] = 1500  # Pitch neutral
    rc_motors[3] = 1500  # Yaw neutral
    
    # Создаем payload из RC каналов (каждый канал - 2 байта little-endian)
    payload = b''
    for channel in rc_motors:
        payload += struct.pack('<H', channel)  # <H = unsigned short (2 bytes) little-endian
    
    # Отправка RC команд
    packet = create_msp_request(MSP_SET_RAW_RC, payload)
    # for byte in packet:
    #     sys.stdout.buffer.write(bytes([byte]))
    #     time.sleep_us(50)
    sys.stdout.buffer.write(packet)
# ========== ФУНКЦИИ ДЛЯ КАМЕРЫ И ВЕБ-СЕРВЕРА ==========
...
def init_camera():
    ...
    try:
        try:
            ...
            pass  # Если камера не была инициализирована, игнорируем ошибку
        camera.init(0, format=camera.JPEG, framesize=camera.FRAME_VGA)
        print("Camera initialized successfully")
        return True
    except Exception as e:
        print(f"Camera init failed: {e} - continuing without camera")
        return False
...
<head>
    ...
    <style>
        ...
        #video-stream { max-width: 100%; border: 1px solid #ccc; background: #f0f0f0; }
...
def send_imu_data(client):
    """Отправка данных IMU в формате JSON"""
    import json
    response = json.dumps(current_imu)
...
def handle_command(client, request):
    """Обработка команд"""
    global blink_enabled, motor_enabled, action_counter
    
    if 'type=blink' in request:
        blink_enabled = True
        action_counter = 0
        response = "Command executed"
    elif 'type=motors' in request:
        motor_enabled = True
        action_counter = 0
        response = "Command executed"
    else:
...
"""
    client.send(headers.encode() + response.encode())
streaming_lock = _thread.allocate_lock()

def streaming_worker():
    """Рабочий поток для потоковой передачи с обработкой ошибок"""
    global streaming_active, stream_client
    
    frame_counter = 0
    max_frames_without_client = 5  # Максимум кадров без подтверждения клиента
    ...
    while streaming_active:
        try:
            ...
            try:
                ...
                print(f"Camera capture error: {capture_error}")
...
"""
                try:
                    # Пытаемся отправить кадр
                    with streaming_lock:
                        stream_client.send(frame_header.encode())
                        stream_client.send(img_data)
                        stream_client.send(b'\r\n')
                    
                    frame_counter = 0  # Сброс счетчика при успешной отправке
                    
                except OSError as e:
                    if e.errno == 104:  # ECONNRESET - клиент отключился
                        print("Client disconnected from stream")
                        break
                    elif e.errno == 32:  # EPIPE - broken pipe
                        print("Stream pipe broken")
                        break
                    else:
                        print(f"Stream send error: {e}")
                        frame_counter += 1
                        if frame_counter >= max_frames_without_client:
                            print("Too many send errors, stopping stream")
                            break
                except Exception as e:
            ...
            print(f"Stream worker critical error: {e}")
            break
...
def start_video_stream(client):
    """Начало потоковой передачи видео в отдельном потоке"""
    global streaming_active, stream_client
    
    # Если уже стримим, останавливаем предыдущий поток
    if streaming_active and stream_client:
        streaming_active = False
...
"""
        ...
        _thread.start_new_thread(streaming_worker, ())
        print("Stream started in separate thread")
...
def handle_streaming():
    ...
    if not streaming_active or stream_client is None:
        return
...
def handle_client(client, request):
    ...
    try:
        ...
        elif request.startswith('GET /imu'):
            send_imu_data(client)
        elif request.startswith('GET /command'):
...
def msp_task():
    """Задача обработки MSP данных"""
    global current_imu, blink_enabled, motor_enabled, action_counter
    
    # Обработка входящих MSP сообщений
    msp_packets = read_serial_non_blocking()
    
    for packet_data in msp_packets:
        parsed = parse_msp_response(packet_data)
        if parsed:
            if parsed['command'] == MSP_RAW_IMU and parsed['payload_size'] == 18:
                imu_data = parse_imu_data(parsed['payload'])
                if imu_data:
                    # Обновляем глобальные данные IMU
                    current_imu.update({
                        'accel_x': imu_data['accel']['x']/512,
                        'accel_y': imu_data['accel']['y']/512,
                        'accel_z': imu_data['accel']['z']/512,
                        'timestamp': time.ticks_ms()
                    })
                    
            elif parsed['command'] == MSP_SONAR_ALTITUDE:
                sonar_data = bytes_to_int32_le(parsed['payload'], 0)
                current_imu['sonar'] = sonar_data  # Исправлено: используем квадратные скобки
...
def main_loop():
    ...
    last_imu_request = 0
    last_sonar_request = 0
    imu_update_interval = 500
    sonar_update_interval = 550
    rc_update_interval = 50
    last_blink = 0
    last_rc_write = 0
    last_frame = 0
    Key_board_counter = 0
        ...
        try:
            ...
            for sock, event in events:
                if sock == server_socket:
                    # Новое подключение к веб-серверу
            ...
            # Отправка MSP запросов по таймеру
            if time.ticks_diff(current_time, last_rc_write) > rc_update_interval:
                rc_control_task()
                last_rc_write = current_time
              
            elif time.ticks_diff(current_time, last_imu_request) > imu_update_interval:
                packet = create_msp_request(MSP_RAW_IMU)
                sys.stdout.buffer.write(packet)
                last_imu_request = current_time

            elif time.ticks_diff(current_time, last_sonar_request) > sonar_update_interval:
                packet = create_msp_request(MSP_SONAR_ALTITUDE)
                sys.stdout.buffer.write(packet)
                last_sonar_request = current_time
          
            # Мигание светодиодом
            if time.ticks_diff(current_time, last_blink) > 5000:
                LED_PIN.value(not LED_PIN.value())
                last_blink = current_time
            
            # time.sleep_ms(10)
        except KeyboardInterrupt:
            print(Key_board_counter)
            # Key_board_counter += 1
            if Key_board_counter < 100:
                continue    
            else:
                break
        except Exception as e:
